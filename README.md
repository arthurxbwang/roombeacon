...
**开发中：改期后重新签到和日历自动核验代码已完成，尚未部署。现有应用缺少日历读取权限；自动测试所需权限、专用日历和工具见[修复说明](docs/v5-reschedule-autoverify.md)。**

面向飞书会议室的独立门牌系统。服务器集中采集并缓存日程，终端显示会议状态、时间、组织者、后续会议与官方签到二维码。

**当前Control：默认V4，保留V1–V3；V5仅IT灯塔-Test试点。** 后端7deb6ba、静态da63152，已实测非重复及每日重复单实例的签到、自动释放与后续预约保留；门牌提前结束已关闭，其他342间保持官方方案。新增预约仍需按类型逐实例登记，重复改期、周/月重复和多日长稳尚未实测。见[重复实例验收](docs/v5-recurring-release-verification.md)及[部署回退](docs/v5-control-deployment.md)。软件仓库版本暂为0.1.0，页面版本不表示正式发布版本。
