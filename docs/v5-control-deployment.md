...
> 2026-09-21 当前后端为 `c6658591d59052fe9673f5bd37d4163d2e9a1056`，静态仍为85a3cf2。仅IT灯塔-Test具备V5写入，其余342间官方。已完成非重复预约一次真实自动释放、一次真实提前结束及释放后及时刷新；本轮两次后端切换和回退见[夜间排查](v5-overnight-verification.md)及[四场实测](v5-four-bookings-verification.md)。
...
## Control 单次点击页面更新（2026-09-20）

- 静态 SHA：`c56365391c949a3001c93cc1e37ca9978f4e09ca`；Nginx 根指向该发布目录 `frontend/dist`。后端仍运行 `f3a07ea28368a7203ac218292a5901eb980fd0d2`，PID 和工作目录核验未变，未重启采集或释放任务。
- 仅修正 Control 操作提示与流程：取消进入测试的额外点击，白名单内直接显示真实提交按钮；无预约明确提示，成功时间只来自服务器审计。普通查询不签到、不保活。
- TypeScript/Vite 构建、Chromium 67 项通过（含先失败后修复的单次点击、会议结束后查询回执用例）；后端未变，沿用此前 146 项验证。线上 HTML SHA、JS/CSS 哈希、目标房间开关核验通过。
- 初次切换在即时 HTML 检查未通过时自动恢复旧 Nginx；重试增加 reload 后等待，再核对成功。未触及真实会议签到状态。
- 回退备份 `/var/backups/roombeacon-control/20260920-c563653-static-retry/nginx.conf`；恢复此配置并 `nginx -t`、reload 即恢复前次静态页面，后端和 Redis 保持不变。
