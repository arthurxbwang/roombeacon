...
自动核验代码已随生产 `3359332` 部署；本轮只读核对生效日历映射数量为 0，写入仅有 1 间房白名单。仍缺专用日历读取与准确映射、配置启用和真实改期联调，接续 [Issue #2](https://github.com/arthurxbwang/roombeacon/issues/2)。下面“尚未部署”和旧服务器版本是 2026-09-21 的历史，历史权限错误也不代表本轮重新调用飞书的结果。

当前业务参数通过软件模板发布版本并显式部署；房间核验资格与预约实例证据独立，模板不复制资格。业务主控设备参与签到和保活，展示屏不能代替。启用时基于最新生产版本准备配置变更及回退；若代码无变化，不重复部署旧实现。回退通常移除新增日历映射并保留保护记录；模板回退按 [ADR 0005](adr/0005-versioned-configuration.md) 选择兼容版本组合。下方恢复 `7deb6ba`/`da63152` 的步骤不是当前生产回退指令。
