...
const props = defineProps<{ device: ManagedDevice; rooms: Room[]; editable: boolean; profiles: DeviceProfile[] }>()
...
function save() {
  ...
  run(() => management('PUT', '/api/v6/admin/devices/'+props.device.id, abort.signal,
    {expected_revision:revision.value,room_id:room.value,status:status.value,config:config.value,confirm_model_mismatch:confirmed}))
}
