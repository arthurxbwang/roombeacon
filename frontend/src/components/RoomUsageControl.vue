...
const props = defineProps<{ roomId: string; roomName: string; token: string }>()
...
async function run(action: 'load' | 'save' | 'verify' | 'pause' | 'resume') {
  ...
  try {
    if (action === 'save' && policy.value) await usageRequest('PUT', path + '/policy', props.token, abort.signal, policy.value)
...
<template>
  <section class="usage-control" aria-label="V5 使用规则">
    ...
    <form v-if="policy" @submit.prevent="run('save')">
      <label>签到方案<select v-model="policy.owner" @change="policy.mode = 'off'"><option value="official">V4 · 飞书官方</option><option value="v5">V5 · 平板与服务器</option></select></label>
      ...
      <label><input v-model="policy.native_policy_cleared" type="checkbox" />已关闭本房间飞书未签到自动释放</label>
      <label><input v-model="policy.release_verified" type="checkbox" />已在专用预约验证公开释放接口及影响范围</label>
      <button :disabled="busy">保存房间规则</button>
    </form>
    <template v-if="usage">
      <p>服务端写入：{{ writes ? '已开放' : '未开放' }} · 全局释放：{{ usage.paused ? '已暂停' : '未暂停' }}</p>
      <div class="buttons"><button :disabled="busy" @click="run('pause')">暂停所有房间释放</button><button :disabled="busy || !writes" @click="run('resume')">解除全局暂停</button></div>
      <div v-if="usage.record" class="verify">
