...
import {defaultConfig,management,managementError,networkLabel,statusLabel,type DeviceConfig,type ManagedDevice,type Manager,type Room} from '@/api/management'
import './v6Control.css'
const abort=new AbortController()
const user=ref<Manager|null>(null), credential=ref(''), error=ref(''), info=ref(''), busy=ref(false), checking=ref(true)
...
const batchConfig=ref<DeviceConfig>({...defaultConfig}), checked=ref<string[]>([])
...
async function load() {
  ...
  loading=true
  try {
    const [list,directory,presets]=await Promise.all([
    ...
    devices.value=list;rooms.value=directory;templates.value=presets
...
async function login(){
  ...
  try{await management('POST','/api/v6/auth/session',abort.signal,{}, {Authorization:`Bearer ${credential.value}`});credential.value='';await identify()}
...
function publishBatch(){
  const targets=Object.fromEntries(devices.value.filter(d=>checked.value.includes(d.id)).map(d=>[d.id,d.revision]))
  if(!Object.keys(targets).length){error.value='请先选择已激活设备';return}
  if(window.confirm(`向 ${Object.keys(targets).length} 台设备发布配置？会议室绑定保持不变。`))operate(()=>management('POST','/api/v6/admin/batch-config',abort.signal,{devices:targets,config:batchConfig.value}))
}
function useTemplate(){const t=templates.value.find(t=>t.id===templateId.value);if(t)batchConfig.value={...t.config}}
onMounted(async()=>{
  if(new URLSearchParams(location.search).has('login_error'))error.value='飞书登录未完成，请核对应用回调配置后重试。'
...
<template>
  ...
  <main v-else class="v6-control">
    ...
    <section v-if="!user" class="v6-login"><p class="v6-eyebrow">ROOMBEACON CONTROL</p><h1>管理每一块会议门牌</h1><p class="v6-muted">统一登录、设备激活与远程配置</p><p v-if="checking">正在检查登录状态…</p><template v-else><a v-if="feishu" class="v6-primary-link" href="/api/v6/auth/feishu/start">飞书扫码 / 快捷登录</a><p v-else class="v6-muted">飞书登录待配置，可先使用主控凭证。</p><form @submit.prevent="login"><label>主控凭证<input v-model="credential" type="password" autocomplete="current-password" required placeholder="输入现有主控凭证" /></label><button :disabled="busy">进入管理后台</button></form><button class="v6-text" @click="logout">清除当前登录状态</button></template><p class="v6-error" role="alert">{{ error }}</p></section>
    <template v-else>
      ...
      <section v-if="tab==='devices'" class="v6-card"><div class="v6-toolbar"><input v-model="search" aria-label="搜索设备" placeholder="搜索唯一码、SN、MAC 或会议室" /><select v-model="filter" aria-label="设备状态"><option value="all">全部设备</option><option value="pending">待激活</option><option value="active">已激活</option><option value="revoked">已撤销</option></select></div>
        ...
        <div v-if="editable && checked.length" class="v6-batch"><strong>已选 {{ checked.length }} 台</strong><select v-model="templateId" @change="useTemplate"><option value="">选择配置模板</option><option v-for="t in templates" :value="t.id" :key="t.id">{{ t.name }}</option></select><button :disabled="busy||!templateId" @click="publishBatch">批量下发</button></div>
      </section>
      ...
      <section v-if="tab==='templates'" class="v6-card v6-presets"><h2>配置模板</h2><p class="v6-muted">模板统一显示方案与设备设置，会议室绑定按设备保留。</p><ul><li v-for="t in templates" :key="t.id">{{ t.name }} · {{ t.config.version.toUpperCase() }} · {{ t.config.portrait?'竖屏':'横屏' }} · 灯控{{ t.config.room_light?'开启':'关闭' }}</li></ul><form v-if="editable" @submit.prevent="operate(()=>management('POST','/api/v6/admin/templates',abort.signal,{name:templateName,config:batchConfig}))"><label>模板名称<input v-model="templateName" required maxlength="80" /></label><label>显示方案<select v-model="batchConfig.version"><option value="v6">V6</option><option value="v4">V4</option><option value="v5">V5</option></select></label><label>方向<select v-model="batchConfig.portrait"><option :value="false">横屏</option><option :value="true">竖屏</option></select></label><label class="v6-check"><input type="checkbox" v-model="batchConfig.room_light" />同步侧边灯</label><button :disabled="busy">保存模板</button></form></section>
      ...
      <section v-if="tab==='audit'" class="v6-card v6-table-wrap"><table><thead><tr><th>时间</th><th>操作者</th><th>操作</th><th>目标</th></tr></thead><tbody><tr v-for="log in logs" :key="log.id"><td>{{ new Date(log.time*1000).toLocaleString() }}</td><td>{{ log.actor.slice(0,16) }}</td><td>{{ log.action }}</td><td>{{ log.target.slice(0,16) }}</td></tr></tbody></table></section>
      <footer class="v6-footer">RoomBeacon V6 · 配置自动同步 · Wi-Fi 与有线统一管理</footer>
    </template>
    <V6DevicePanel v-if="selected" :device="selected" :rooms="rooms" :editable="editable" @close="selected=null" @changed="load" />
  </main>
</template>
